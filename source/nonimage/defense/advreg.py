...
sys.path.append(os.path.join(FILE_DIR, '../../'))
SAVE_ROOT = os.path.join(FILE_DIR, '../../../results/%s/advreg')
...
def parse_arguments():
    ...
    parser.add_argument('--num_batches_target', type=int, default=2, help='number of target training batches')
    parser.add_argument('--num_batches_attack', type=int, default=50, help='number of attack training batches')
...
def main():
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if epoch < 1:
            ...
            for i in range(len(trainer.trainloader) // args.num_batches_target):
                attack_loss, attack_acc = trainer.train_attack(model, attack_model, attack_optimizer, args.num_batches_attack)
                train_loss, train_acc, train_acc5 = trainer.train_privately(model, attack_model, optimizer, args.num_batches_target)
            test_loss, test_acc, test_acc5 = trainer.test(model)
            attack_test_loss, attack_test_acc = trainer.test_attack(model, attack_model)
