...
def parse_arguments():
    ...
    parser.add_argument('--stop_eps', type=int, nargs='+', default=[5, 10, 15, 20, 30, 40, 50, 75, 100, 150, 200, 250],
                        help='(stopping) epochs')
    return parser
...
def main():
    ...
    ### Training
    for epoch in range(start_epoch, min(args.num_epochs, max(args.stop_eps))):
        ...
        if epoch + 1 in args.stop_eps:
            ep = epoch + 1
            mkdir(os.path.join(save_dir, 'ep%d' % ep))
            write_yaml(vars(args), os.path.join(save_dir, 'ep%d' % ep, 'params.yml'))
            torch.save(model, os.path.join(save_dir, 'ep%d' % ep, 'model.pt'))
            train_losses, test_losses = trainer.get_loss_distributions(model)
            plot_hist([train_losses, test_losses], ['train', 'val'], os.path.join(save_dir, 'hist_ep%d.png' % ep))
